...
###Description:
*Weak white noise* processes are fixed with a constant mean and constant variance and do not have a correlation over time. In other words, if the mean is not constant over time, the variance changes over time, or there seems to be a correlation between patterns in the data across time, then it is not White Noise.

###Simulate WN Model
```{r}
#You can simulate WN model data by using arima.sim, which stands for autoregressive integrated moving average. Broad type of time series model that uses WN model as a case.
...
# sigma^2 is an estiamte of the variance. The sd is the square root of this variance estimate.
...
##Practice Simulating and Estimating WN Models
